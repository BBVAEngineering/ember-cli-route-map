...
module.exports = BuildCommand.extend({
	...
	run(commandOptions) {
		...
		if (commandOptions.output) {
			const outputDir = resolveDir(commandOptions.output || '');

			fs.writeFileSync(path.join(outputDir, 'route-map.json'), JSON.stringify(result), 'utf8');
			...
			if (commandOptions.pretty) {
				...
				fs.writeFileSync(path.join(outputDir, 'tree.html'), treeTemplate({ result: JSON.stringify(result) }), 'utf8');
				fs.writeFileSync(path.join(outputDir, 'table.html'), tableTemplate({ result: JSON.stringify(result) }), 'utf8');
			}
		} else {
